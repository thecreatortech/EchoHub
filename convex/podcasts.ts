...
export const updatePodcastViews = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.db.patch(args.podcastId, {
			views: (podcast.views || 0) + 1, // Ensure views is incremented
		});
	},
});
...
// This query will get the latest podcasts, ordered by creation time (or another field).
export const getLatestPodcasts = query({
	handler: async (ctx) => {
		// Fetch all podcasts and order by the creation time or an equivalent timestamp field.
		const latestPodcasts = await ctx.db
			.query('podcasts')
			.order('desc') // Assuming 'desc' sorts by creation or upload time
			.take(8); // Limit to the latest 8 podcasts
